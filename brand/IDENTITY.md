...
Interactive help carries the Wonder Wagon line grammar: letterspaced
`L O R E K E E P E R` in the dark `--lk-accent`, followed by a dimmed version
and `LK-047`. A successful interactive `lore init` is bookended by the approved
five-row terminal translation of the ring and four-point star. Failed init,
pipes, JSON, `--version`, and machine-readable output keep their previous bytes.

- Periwinkle draws the ring and name; gilt is reserved for the star. At the
  sixteen-colour floor they become bright blue and yellow respectively.
- `NO_COLOR` removes ANSI escapes without discarding the mark. `WW_ASCII=1`
  independently selects the ASCII ring and star for terminals whose glyph
  widths are ambiguous. Narrow terminals fall back to the one-line grammar.
- Colour never carries meaning. Severity, where the CLI ever shows one, uses
  the terminal's own green, cyan, yellow, and red.
- On a light terminal the accent measures 2.39:1 against white, below the 3:1
  ...
  is lost. Terminal background colour cannot be detected reliably; `NO_COLOR`
  is the documented escape hatch.
- Box-drawing characters and `✦` can have ambiguous width under East Asian/CJK
  terminal settings. `WW_ASCII=1` is the deterministic compatibility mode.

`packages/cli/src/identity.ts` is generated and committed by
`brand/terminal/build.mjs`. Lorekeeper owns its geometry and palette;
`wonder-wagon-ui/cli` owns capability detection, degradation, spacing, and the
emitted module structure. `npm run brand:terminal:check` fails if the artifact
is stale. Only the generator reads Wonder Wagon UI; the shipped CLI has no
