...
Lorekeeper edits files a person wrote by hand and may not have backed up. The
formatting of those files is user-authored content, not incidental.

**Reads may parse freely.** Parsing frontmatter into an object to query it is
fine, and a YAML parser is an acceptable dependency for that.

**Writes must preserve user-authored representation.** Never serialize a parsed
object back over a user's file. Mutate the smallest span of text that achieves
the change and leave every other byte alone.

This is not a style preference. Prototype `prototypes/schema-v0` measured an
object parse-then-re-dump against 16 realistic files and altered all 16:

- `created: 2026-08-21T09:02:00-05:00` became `2026-08-21T14:02:00.000Z`,
  silently discarding the author's local offset
- inline arrays were restructured into block sequences
- YAML comments were dropped entirely
- quoting was normalized away

Surgical line-level editing round-tripped all 16 byte-identically.

Every deterministic mutation carries two tests:

1. **Minimal diff** — the change touches only the lines it must.
2. **Idempotency** — running the same operation twice changes nothing the
   second time.

Round-trip identity (read then write with no mutation produces a byte-identical
file) is a precondition for any code that writes into a brain.
