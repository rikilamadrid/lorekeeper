...
Feature specs are planning records. Tickets are the executable unit of work.
...
This project generates and modifies files that hold a person's notes. Those
files are the product's whole reason to exist and are frequently not backed up.

Ask before:

- writing, moving, or deleting anything inside a generated brain
- any operation that would overwrite a file the managed manifest does not own
- adding a network call, credential, telemetry, or AI provider to the CLI —
  v0.1 is deliberately offline and vendor-free
- changing the frontmatter schema or the provenance contract once a Feature
  depends on it
- adding an embedding, vector, or semantic-search dependency

Never place private brain content, real notes, or personal data in this
repository, including in tests and fixtures. Use synthetic fixtures.
